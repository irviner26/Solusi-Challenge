...
public class ProductController {
    ...
    public ResponseEntity<ErrorResponse<Object>> requestAddProduct(@RequestParam("merchant") String merchantName,
        ...
        if(merchantService.merchantOfName(merchantName).get().getMerchantAddress().isEmpty()) {
    ...
    public ResponseEntity<ErrorResponse<Object>> requestUpdateProductName(@PathVariable("oldName") String prodOldName,
                                           @RequestBody ProductResponse productResponse,
                                           @RequestParam("merchant") String merchantName, @RequestHeader(name = "Authorization") String token) throws ExecutionException, InterruptedException {
        if(merchantService.merchantOfName(merchantName).get().getMerchantAddress().isEmpty()) {
            return new ResponseEntity<>(ErrorResponse.builder()
                    .errorCode(HttpStatus.NOT_ACCEPTABLE.value())
                    .errorMessage("Please define merchant "+merchantName+" location")
        ...
        if (merchantService.merchantObjectWithName(merchantName).get().getUser().equals(userService.getUserByName(jwtUtils.getUsernameFromJwtToken(token)))) {
            try {
                if (productService.updateProductName(merchantName,
                        prodOldName,
                        productResponse.getProductName()).get()) {
                    return new ResponseEntity<>(ErrorResponse.builder()
                            .entity(productResponse)
        ...
        if (merchantService.merchantObjectWithName(merchantName).get().getUser().equals(userService.getUserByName(jwtUtils.getUsernameFromJwtToken(token)))) {
            try {
                if (productService.updateProductPrice(merchantName
                        ,prodName
                        ,productResponse.getProductPrice()).get()) {
                    return new ResponseEntity<>(ErrorResponse.builder()
                            .entity(productResponse)
    ...
    public ResponseEntity<ErrorResponse<Object>> requestDeleteProduct(@RequestParam("merchant") String merchantName,
                                       @PathVariable("prodName") String prodName, @RequestHeader(name = "Authorization") String token) throws ExecutionException, InterruptedException {
        if(merchantService.merchantOfName(merchantName).get().getMerchantAddress().isEmpty()) {
            return new ResponseEntity<>(ErrorResponse.builder()
                    .errorCode(HttpStatus.NOT_ACCEPTABLE.value())
                    .errorMessage("Please define merchant "+merchantName+" location")
        ...
        if (merchantService.merchantObjectWithName(merchantName).get().getUser().equals(userService.getUserByName(jwtUtils.getUsernameFromJwtToken(token)))) {
            try {
                if (productService.removeProductOf(prodName,merchantName).get()) {
                    return new ResponseEntity<>(ErrorResponse.builder()
    ...
    public List<ProductResponse> requestGetProduct(@PathVariable("merchant") String merchantName,
        ...
        try {
            return productService.ListOfAvailableProduct(merchantName,page).get();
        } catch (InterruptedException | ExecutionException e) {
            throw new RuntimeException(e);
        }
    }
